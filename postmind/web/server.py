...
async def settings_mcp_servers_get(request: Request):
    """Return the mcp_servers list for the active account."""
    from postmind.config import load_account_config
    ...
    return {"servers": servers, "pool_active": pool is not None}
...
async def settings_mcp_servers_test(request: Request):
    ...
    # Return a stub response — real MCP session requires the optional mcp extra
    return {
        "connected": False,
        "tools": [],
        "error": "MCP test requires the optional [mcp] extra: pip install 'postmind[mcp]'",
    }
